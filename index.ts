...
export async function createEndpointsAndModels(options: GeneratorOptions) {
    ...
    fs.writeFileSync(path.join(options.outDir, `endpoints.ts`), notice.concat(result));

    generateTypeScriptInterfacesForDtoModels(path.join(options.outDir, "models"), ...components);
}
...
function generateTypeScriptInterfacesForDtoModels(modelsDir: string, ...components: any[]) {
    ...
    for (const component of components) {
        ...
        for (const [name, schema] of Object.entries(component.schemas)) {
            ...
            interfaceString += '}\n';
            fs.writeFileSync(path.join(modelsDir, `${interfaceName}.ts`), notice.concat(importStatements).concat(interfaceString));
        }
...
const notice: string = `// This file is generated using automated tool provided by NightmareGaurav (https://github.com/nightmaregaurav)
// Do not edit this file manually except for the lines annotated with a comment that ask you to fill in some details.
`;

const template_string: string = `import axios, {AxiosRequestConfig, AxiosResponse, Method} from "axios";
