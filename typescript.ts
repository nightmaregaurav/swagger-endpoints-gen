        ...
        for (let endpointUrl in swaggerPaths) {
            ...
            for (let method in swaggerEndpoints) {
                ...
                classDefinition += `    static ${endpointName} = class {\n`
                    ...
                    + `        static call = async (${argsString ? argsString + ", " : ""}${callDataParam === "" ? "" : callDataParam + ", "}${queryArgsString === "" ? "" : queryArgsString + ", "}cacheExpiry?: Date, onError?: false | ((error: AxiosError) => void)) : Promise<AxiosResponse<${resDataType}>> => {\n`
                    + `            const url = new URL(this.getUrl(${argsString ? "args" : ""}), baseUrl).toString();\n`
                    + `            return await CallApi<${resDataType}>(url, this.method, ${callDataParam === "" ? "undefined" : "data"}, ${queryArgsString === "" ? "undefined" : " params"}, cacheExpiry, onError);\n`
                    + `        }\n`
                    + `    }\n`;
            }
        }
...
const getTemplateString: (removeComment: boolean) => string = (removeComment) => `
${removeComment ? "" : "// This file is generated using automated tool provided by NightmareGaurav (https://github.com/nightmaregaurav)"}
${removeComment ? "" : "// Do not edit this file manually except for the lines annotated with a comment that ask you to fill in some details."}
...
async function CallApi<TResponse>(url: string, method: requestMethod, data?: {}, params?: {}, cacheExpiry?: Date, onError?: false | ((error: AxiosError) => void)) : Promise<AxiosResponse<TResponse>> {
